...
public class CityGenerator : MonoBehaviour
{
    ...
    [Range(0.1f, 1f)] public float towerChange = 0.02f;
    [Range(0.1f, 1f)] public float plazaChange = 0.05f;
    [Range(0.1f, 1f)] public float parkChange = 0.1f; 
    void Start()
    {
        Generatecity();
    }
    ...
    void CreateTower (int x , int z)
    {
        ...
        topLight.transform.position = new Vector3(x * blockSize, height + 2, z * blockSize);
    ...
    void CreatePlaza(int x , int z)
    {
        ...
        plaza.GetComponent<Renderer>().material.color = Color.cyan;
    ...
    void CreateStreeLight(int x , int z)
    {
        ...
        pole.GetComponent<Renderer>().material.color = Color.cyan;

        GameObject bulb = GameObject.CreatePrimitive(PrimitiveType.Sphere);

        bulb.transform.position = new Vector3(x * blockSize, 10f, z * blockSize);
        bulb.transform.localScale = Vector3.one * 0.05f;
    ...
    {
        float disFromCenter = Vector2.Distance(new Vector2(x, z), new Vector2(cityWidth, cityHeight));

        if (disFromCenter < cityWidth * 0.2f)
        {
            return new(0.5f, 0.6f, 0.7f);
        }
