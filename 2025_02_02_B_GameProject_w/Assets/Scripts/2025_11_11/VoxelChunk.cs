...
public class VoxelChunk : MonoBehaviour
{
    ...
    bool IsCaveAt(int x , int y , int z)
    {
        ...
        float cave2 = Mathf.PerlinNoise(x * caveScale + 100, z * caveScale * 0.5f);
        float cave3 = Mathf.PerlinNoise(x * caveScale * 0.5f, z * caveScale +200f);
    ...
    int GetTerrainHeight(int worldX, int worldZ)
    {
        ...
        for (int i = 0; i < octaves; i++)
        {
            ...
            amplitude *= persistence;       // 다음 옥타브로 갈수로 진폭 줄인다
            frequency *= persistence;       // 주파수를 조정
        }

        int height = groundLevel + Mathf.RoundToInt(noiseHeight * heightVariation);     // 기본 지면 높여 적용
    ...
    bool IsTransparent (int x , int y , int z )     // 특정 위치가 투명인가?
        ...
        for(int x = 0; x< vertices.Count; x++)
        {
            for (int y = 0; y < chunkHeight; y++)
            {
                for(int z = 0; z <chunkSize; z ++)
                {
                    ...
                    AddBlockFaces(x, y, z, block, vertices, triangles, colors); // 육면 체크
                }
            }
